...
import { startCleanupInterval } from './utils/cacheManager.js';

class NH_starlightsercurity extends Client {
  constructor() {
    super({
      intents: [
  ...
  async start() {
    try {
      startupLog('Starting NH_starlightsercurity...');
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      // Initialize Memory Manager for global cache cleanup
      startupLog('Initializing memory manager...');
      MemoryManager.getInstance().initialize({
        memoryThresholdMB: 500,
        maxMapSize: 50000,
      });
      startCleanupInterval(3600000);
      startupLog('Memory manager initialized');
  ...
  startWebServer() {
    ...
    app.get('/', (req, res) => {
      res.status(200).json({
        message: 'NH_starlightsercurity System Online',
        version: '2.0.0',
        timestamp: new Date().toISOString()
      });
    });
  ...
  setupCronJobs() {
    ...
    // Resume pending timeouts (chunked timeouts > Discord 28d limit)
    try {
      import('./services/pendingTimeoutService.js')
        .then(({ resumeDuePendingTimeouts }) => {
          cron.schedule('*/5 * * * *', () => resumeDuePendingTimeouts(this));
          logger.info('Pending timeouts resume cron initialized (every 5 minutes)');
        })
        .catch((e) => {
          logger.warn('Failed to initialize pending timeout cron job:', e?.message || e);
        });
    } catch (e) {
      logger.warn('Failed to initialize pending timeout cron job:', e?.message || e);
    }
...
try {
  const bot = new NH_starlightsercurity();
...
export default NH_starlightsercurity;
