        ...
        try {
            ...
            try {
                ...
                if (error instanceof StarlightError) {
                    ...
                    logger.debug(`StarlightError [${error.type}]: ${error.message}`, error.context || {});
                } else {
                    logger.error(`[Command Error] /jointocreate failed in Guild ${interaction.guildId} by User ${interaction.user.id}: ${error.message}`, { stack: error.stack, errorCode: error.code || 'UNKNOWN' });
    ...
    try {
        ...
        collector.on('collect', async (buttonInteraction) => {
            try {
                ...
                const userMessage = error instanceof StarlightError
