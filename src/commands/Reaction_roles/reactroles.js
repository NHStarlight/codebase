...
import { handleInteractionError, createError, StarlightError, ErrorTypes } from '../../utils/errorHandler.js';
...
async function handleDashboard(interaction, selectedPanelId) {
    ...
    collector.on('collect', async ci => {
        try {
            ...
            logger.error(`[Command Error] /reactroles failed in Guild ${interaction.guildId} by User ${interaction.user.id}: ${error.message}`, { stack: error.stack, errorCode: error.code || 'UNKNOWN' });
            const msg =
