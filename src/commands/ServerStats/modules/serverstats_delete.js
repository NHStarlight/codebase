...
export async function handleDelete(interaction, client) {
  ...
  try {
    ...
    if (!counterToDelete) {
      await InteractionHelper.safeEditReply(interaction, {
        embeds: [
          errorEmbed(`Counter with ID \`${counterId}\` not found. Use \`/serverstats list\` to see all counters.`),
        ],
      }).catch(logger.error);
      return;
    }
...
export async function performDeletionByCounterId(client, guild, counterId) {
  try {
    ...
    // Then delete channel if it still exists
    ...
    if (channelDeleted) {
      message += `\n**Channel:** ${channel?.name ?? counter.channelId} (deleted)`;
    } else if (channel) {
