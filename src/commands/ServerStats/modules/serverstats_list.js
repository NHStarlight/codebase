...
export async function handleList(interaction, client) {
    ...
    if (!interaction.member.permissions.has(PermissionFlagsBits.ManageChannels)) {
        await InteractionHelper.safeEditReply(interaction, { 
            embeds: [errorEmbed("You need **Manage Channels** permission to view Serverstats.")]
        }).catch(logger.error);
        return;
    }
    ...
    try {
        ...
        if (validCounters.length === 0) {
            const embed = createEmbed({
                ...
                description: "No Serverstats have been set up for this server yet.\n\nUse `/Serverstats create` to set up your first Serverstats!",
        ...
        const embed = createEmbed({
            ...
            description: "Here are all the active Serverstats for this server.\n\nServerstats automatically update every 15 minutes.",
            color: getColor('info')
        });
        ...
        embed.addFields({
            name: "📊 **Statistics**",
            value: `**Total Serverstats:** ${validCounters.length}\n**Active Serverstats:** ${validCounters.filter(c => {
        ...
        embed.addFields({
            ...
            value: "`/Serverstats create` - Create new Serverstats\n`/Serverstats update` - Update existing Serverstats\n`/Serverstats delete` - Delete Serverstats",
            inline: false
        });
        ...
        logger.error(`[Command Error] /unknown failed in Guild ${interaction.guildId} by User ${interaction.user.id}: ${error.message}`, { stack: error.stack, errorCode: error.code || 'UNKNOWN' });
        await InteractionHelper.safeEditReply(interaction, {
            embeds: [errorEmbed("An error occurred while fetching serverstats. Please try again.")]
        }).catch(logger.error);
    }
}
