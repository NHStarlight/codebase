...
const BUTTON_TIMEOUT = 300000;
                ...
                collector.on("collect", async (i) => {
                    try {
                        ...
                        logger.error(`[Command Error] /calculate failed in Guild ${interaction.guildId} by User ${interaction.user.id}: ${error.message}`, { stack: error.stack, errorCode: error.code || 'UNKNOWN' });
                        if (!i.deferred && !i.replied) {
                            await i.followUp({
