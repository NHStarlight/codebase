        ...
        .addComponents(
            new ActionRowBuilder().addComponents(
                new TextInputBuilder()
                    ...
                    .setMaxLength(2048)
                    .setRequired(false)
                    .setPlaceholder('Built with NH_starlightsecurity'),
            ),
            new ActionRowBuilder().addComponents(
                new TextInputBuilder()
        ...
        try {
            ...
            collector.on('collect', async ci => {
                try {
                    ...
                    logger.error(`[Command Error] /embedbuilder failed in Guild ${interaction.guildId} by User ${interaction.user.id}: ${error.message}`, { stack: error.stack, errorCode: error.code || 'UNKNOWN' });
                    const msg =
                        error instanceof StarlightError
