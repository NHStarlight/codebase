...
const collector = response.createMessageComponentCollector({ filter, time: 300000 });
            ...
            collector.on('collect', async (i) => {
                try {
                    ...
                    logger.error(`[Command Error] /randomuser failed in Guild ${interaction.guildId} by User ${interaction.user.id}: ${error.message}`, { stack: error.stack, errorCode: error.code || 'UNKNOWN' });
                    await i.reply({
            ...
            collector.on('end', async () => {
                ...
                await InteractionHelper.safeEditReply(interaction, { components: [disabledRow] }).catch(console.error);
            });
