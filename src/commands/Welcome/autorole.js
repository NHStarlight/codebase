    ...
    const { options, guild, client } = interaction;
        ...
        if (subcommand === 'add') {
            ...
            try {
                ...
                logger.error(`[Command Error] /autorole failed in Guild ${interaction.guildId} by User ${interaction.user.id}: ${error.message}`, { stack: error.stack, errorCode: error.code || 'UNKNOWN' });
                await InteractionHelper.safeEditReply(interaction, {
                    embeds: [errorEmbed(
                        'Add Failed',
