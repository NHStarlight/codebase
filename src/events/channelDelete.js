...
export default {
    ...
    async execute(channel, client) {
        ...
        // Anti-Nuke handling (best-effort, non-blocking) — runs for ALL channel types
        try {
            ...
            for (let attempt = 0; attempt < 2; attempt += 1) {
                ...
                if (matchedExecutor) {
                    ...
                    await antiNuke.handleEvent(channel.guild, matchedExecutor, { type: 'channel', data: channel, targetSnapshot: snapshot });
                    break;
                }
            ...
            if (!matchedExecutor) {
                logger.debug(`Anti-Nuke: No matching audit entry found for deleted channel ${channel.id} guild=${guildId}`);
            }
        } catch (e) {
            logger.warn(`Anti-Nuke channelDelete handler failed for guild ${guildId}:`, e?.message || e);
        }

        // Only guard downstream logic (JTC, counters) for voice (2) and category (4)
