...
export default {
  ...
  async execute(member) {
    try {
        ...
        // =====================
        // SECURITY FORTRESS: Anti-Raid State Machine Detection
        // =====================
        try {
          const antiNuke = AntiNukeService.getInstance();
          const settings = antiNuke.getSettings(guild.id);
          if (settings?.isEnabled) {
            await antiNuke.processRaidDetection(guild, member);
          }
        } catch (e) {
          logger.warn(`Anti-raid detection error in guild ${guild.id}:`, e?.message || e);
        }
