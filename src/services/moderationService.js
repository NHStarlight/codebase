...
export class ModerationService {
  ...
  /**
   * Check if the target is the guild owner (protected from moderation).
   * @param {import('discord.js').Guild} guild
   * @param {string} targetId
   * @returns {boolean}
   */
  static isOwnerProtected(guild, targetId) {
    return guild?.ownerId === targetId;
  }
  ...
  static validateHierarchy(moderator, target, action) {
    ...
    // AUDIT FIX: Prevent actions against the server owner
    const targetUserId = target.id || target.user?.id;
    if (ModerationService.isOwnerProtected(moderator.guild, targetUserId)) {
      return {
        valid: false,
        error: `You cannot ${action} the server owner.`,
      };
    }

    // Guild owner of moderator bypasses hierarchy (they rule the server)
  ...
  static async banUser({
    ...
    try {
      ...
      if (targetMember) {
        ...
        if (!botCheck.valid) {
          throw new StarlightError(botCheck.error, ErrorTypes.PERMISSION, botCheck.error);
        }

        const modCheck = this.validateHierarchy(moderator, targetMember, 'ban');
        if (!modCheck.valid) {
          throw new StarlightError(modCheck.error, ErrorTypes.PERMISSION, modCheck.error);
        }
      } else {
  ...
  static async removeTimeoutUser({
    ...
    try {
      ...
      // AUDIT FIX: Add owner protection to untimeout
      const modCheck = this.validateHierarchy(moderator, member, 'untimeout');
      if (!modCheck.valid) {
        throw new StarlightError(modCheck.error, ErrorTypes.PERMISSION, modCheck.error);
      }
