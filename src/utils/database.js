...
class DatabaseWrapper {
    ...
    async initialize() {
        ...
        try {
            ...
            if (isProduction) {
                const reason = pgFailure?.reason || 'POSTGRES_UNAVAILABLE';
                const message = pgFailure?.message || 'PostgreSQL connect() returned false';
                const fatalError = new Error(
                    `PostgreSQL unavailable in production (reason: ${reason}). Refusing to start.`
                );
                fatalError.code = 'POSTGRES_UNAVAILABLE';
                fatalError.details = { reason, message };
                throw fatalError;
            }
        } catch (error) {
            ...
            if (isProduction) {
                const fatalError = new Error(
                    `PostgreSQL unavailable in production. Refusing to start. ${error.message}`
                );
                fatalError.code = 'POSTGRES_UNAVAILABLE';
                fatalError.details = { original: error };
                throw fatalError;
            }
        }
