 ...
 * 
 * This module provides structured error handling for the NH_starlightsecurity application.
 * 
 * PHILOSOPHY:
 ...
 * 
 * USAGE:
 * - Throw StarlightError for application-specific errors
...
export function createError(message, type = ErrorTypes.UNKNOWN, userMessage = null, context = {}) {
    ...
    return new StarlightError(message, type, userMessage, normalizedContext);
}

/**
 * Safely reply to an interaction, handling edge cases like expired interactions
 * or already-replied interactions without throwing.
 * 
 * @param {import('discord.js').Interaction} interaction - The Discord interaction
 * @param {string|Object} content - String message or embed object
...
export async function safeReply(interaction, content, ephemeral = true) {
    if (!interaction) {
        logger.warn('[safeReply] Interaction is null/undefined');
        return false;
    }

    try {
        // Check if interaction is expired (> 15 minutes)
        if (interaction.createdTimestamp && (Date.now() - interaction.createdTimestamp) > 14.5 * 60 * 1000) {
            logger.warn('[safeReply] Interaction expired, cannot reply');
            return false;
        }

        const options = typeof content === 'string'
            ? { content, ephemeral }
            : { ...content, flags: ephemeral ? (content.flags ?? (1 << 6)) : content.flags };
        ...
        // Known "safe" Discord errors for expired interactions
        if (error?.code === 10062 || error?.code === 40060) {
            logger.debug('[safeReply] Interaction already expired or acknowledged');
            return false;
        }
        logger.warn('[safeReply] Failed to reply:', error?.message || error);
        return false;
    }
}
...
export async function safeDeferReply(interaction, ephemeral = true) {
    ...
    try {
        ...
        logger.warn('[safeDeferReply] Failed:', error?.message || error);
        return false;
    }
}

/**
 * Safely follow up on an interaction
 * @param {import('discord.js').Interaction} interaction
 * @param {string|Object} content
 * @param {boolean} [ephemeral=true]
 * @returns {Promise<boolean>}
 */
export async function safeFollowUp(interaction, content, ephemeral = true) {
    if (!interaction) return false;
    ...
    try {
        ...
        logger.warn('[safeFollowUp] Failed:', error?.message || error);
        return false;
    }
}
