...
class PostgreSQLDatabase {
        ...
        this._healthCheckTimer = setInterval(async () => {
            try {
                ...
                if (!this.isConnected) {
                    ...
                    logger.info('[PostgreSQL] Health check: connection restored');
                }
            } catch (err) {
    ...
    _scheduleReconnect() {
        ...
        if (this._reconnectAttempts >= this._maxReconnectAttempts) {
            logger.error('[PostgreSQL] Max reconnection attempts reached. Giving up.');
            this._isReconnecting = false;
            this._reconnectAttempts = 0;
            return;
        }
        ...
        logger.warn(
            `[PostgreSQL] Scheduling reconnect attempt ${this._reconnectAttempts}/${this._maxReconnectAttempts} in ${Math.round(delay / 1000)}s...`
        );
        ...
        this._reconnectTimer = setTimeout(async () => {
            try {
                logger.info('[PostgreSQL] Attempting reconnection...');
                
                // Close old pool if still exists
                if (this.pool) {
                    try {
                        await this.pool.end();
                    } catch (endErr) {
                        logger.warn('[PostgreSQL] Error ending old pool:', endErr.message);
                    }
                    this.pool = null;
                }
                ...
                if (this.isConnected) {
                    ...
                    // Re-setup listeners on new pool
                    this._setupPoolListeners();
                } else {
                    logger.warn('[PostgreSQL] Reconnection failed, will retry...');
                    this._isReconnecting = false;
                    this._scheduleReconnect();
                }
            } catch (err) {
                logger.error('[PostgreSQL] Reconnection error:', err.message);
                this._isReconnecting = false;
                this._scheduleReconnect();
            }
        }, delay);
    ...
    async _establishConnection() {
        ...
        for (let attempt = 1; attempt <= attempts; attempt += 1) {
            try {
                ...
                // Set up pool event listeners and health check
                this._setupPoolListeners();
    ...
    async createTables() {
        const tables = [
            ...
            `CREATE TABLE IF NOT EXISTS guild_settings (
                ...
                punishment_type VARCHAR(20) DEFAULT 'quarantine',
                quarantine_role_id VARCHAR(20),
                log_channel_id VARCHAR(20),
                honeypot_channel_id VARCHAR(20) DEFAULT NULL
            )`,
            ...
            `CREATE TABLE IF NOT EXISTS punished_users (
                ...
                guild_id VARCHAR(20) NOT NULL,
                user_id VARCHAR(20) NOT NULL,
                old_roles JSONB NOT NULL,
                punished_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                CONSTRAINT unique_guild_punished UNIQUE (guild_id, user_id)
            )`,

            `CREATE INDEX IF NOT EXISTS idx_punished_users_guild_id ON punished_users(guild_id)`,
            `CREATE INDEX IF NOT EXISTS idx_punished_users_user_id ON punished_users(user_id)`,
    ...
    async createIndexes() {
        const indexes = [
            ...
            `CREATE INDEX IF NOT EXISTS idx_guild_settings_honeypot ON guild_settings(honeypot_channel_id)`
        ];
